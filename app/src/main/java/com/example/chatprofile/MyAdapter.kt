...
class MyAdapter (private val context: Activity, private val arrayList: ArrayList<User>) :
    ArrayAdapter<User>(context, R.layout.card, arrayList) {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val imgId = view.findViewById<CircleImageView>(R.id.profile_image)

        name.text = arrayList[position].name
        lastMsg.text = arrayList[position].lastMsg
        lastMsgTime.text = arrayList[position].lastMsgTime
        imgId.setImageResource(arrayList[position].imageId)
